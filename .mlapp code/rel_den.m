...
if strcmp(structure_type,'Solid')                                                           % Check if Solid
    ...
    switch lattice_type
        ...
        case "PN Surface"
            isovalue = [-0.999 0.7];
        otherwise
            isovalue = [-0.999 0.999];
    end

    rd = zeros(size(isovalue));                                          % Initialize isovalue for two extreme values -1 and 1
    for i = 1:length(isovalue)                                                              % for-loop for the two isovalues chosen
        [F,V] = imp_model_func(app,out_p,x1,y1,z1,cell_len_x,cell_len_y,...
    ...
    if relDenCheck == true                                                           %check if new approximate thickness is calculated using relative density
        app_t= [0.2 2];
        rd = zeros(size(app_t));

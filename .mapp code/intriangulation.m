function in = intriangulation(app,vertices,faces,testp,heavytest)
...
for n = 1:heavytest+1,
    ...
    if app.stopFlag == 0
    [in,cl] = VOXELISEinternal(app,testp(:,1),testp(:,2),testp(:,3),meshXYZ);
    end 

    % x-direction:
    % has only to be done for those points, that were not determinable in the first step --> cl
    if app.stopFlag == 0
    [in2,cl2] = VOXELISEinternal(app,testp(cl,2),testp(cl,3),testp(cl,1),meshXYZ(:,[2,3,1],:));
    ...
    if app.stopFlag == 0
    [in3,cl3] = VOXELISEinternal(app,testp(cl,3),testp(cl,1),testp(cl,2),meshXYZ(:,[3,1,2],:));
...
%==========================================================================
function [OUTPUT,correctionLIST] = VOXELISEinternal(app,testx,testy,testz,meshXYZ)
